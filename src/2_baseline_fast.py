...
tf = TfidfVectorizer(max_features=10000, ngram_range=(1, 2), min_df=2)  # Reduced from 50k to 10k
...
kf = KFold(n_splits=3, shuffle=True, random_state=SEED)  # Reduced from 5 to 3 folds
...
params = {
    ...
    'num_leaves': 64,      # Reduced from 128
    'seed': SEED,
    'verbose': -1,
    'n_jobs': -1,
}
...
for fold, (tr_idx, val_idx) in enumerate(kf.split(X)):
    ...
    model = lgb.train(
        ...
        dtrain,
        valid_sets=[dval],
        num_boost_round=500,  # Reduced from 2000
        callbacks=[lgb.early_stopping(stopping_rounds=20, verbose=False)]  # Reduced from 50
    )
    ...
    if fold == kf.n_splits - 1:
        joblib.dump(model, f"{MODELS_DIR}lgb_baseline_fast.pkl")
...
train_sample[['sample_id', 'oof']].to_csv(f"{OUTPUTS_DIR}oof_baseline_fast.csv", index=False)

# Save the final submission file
test['price'] = preds
test[['sample_id', 'price']].to_csv(f"{OUTPUTS_DIR}sub_baseline_fast.csv", index=False)

print(f"\nSubmission file created at: {OUTPUTS_DIR}sub_baseline_fast.csv")
print("Fast baseline script executed successfully!")
